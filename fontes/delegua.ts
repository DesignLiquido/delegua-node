...
} from "@designliquido/delegua/fontes/tradutores";
...
export class Delegua implements DeleguaInterface {
  ...
  versao(): string {
    try {
      ...
      return (
        JSON.parse(
          sistemaArquivos.readFileSync(manifesto, { encoding: "utf8" })
        ).version || "0.21"
      );
    } catch (error: any) {
  ...
  traduzirArquivo(
    ...
    let resultado = null;
    if (
      this.tradutorJavaScript !== null &&
      this.tradutorJavaScript !== undefined
    ) {
      if (this.afericaoErros(retornoImportador)) {
        process.exit(65); // Código para erro de avaliação antes da tradução
      }

      resultado = this.tradutorJavaScript.traduzir(
        retornoImportador.retornoAvaliadorSintatico.declaracoes
      );
    } else if (
      this.tradutorPython !== null &&
      this.tradutorPython !== undefined
    ) {
      if (this.afericaoErros(retornoImportador)) {
        process.exit(65); // Código para erro de avaliação antes da tradução
      }

      resultado = this.tradutorPython.traduzir(
        retornoImportador.retornoAvaliadorSintatico.declaracoes
      );
    } else if (
      this.tradutorReversoJavascript !== null &&
      this.tradutorReversoJavascript !== undefined
    ) {
      resultado = this.tradutorReversoJavascript.traduzir(
        retornoImportador.conteudoArquivo.join("\n")
      );
    } else {
      resultado = this.tradutorVisualg.traduzir(
        retornoImportador.conteudoArquivo.join("\n")
      );
    }
        ...
        [".delegua", ".js", ".alg"].map((extensao) => {
          if (caminhoAbsolutoPrimeiroArquivo.includes(extensao)) {
            sistemaArquivos.writeFile(
              caminhoAbsolutoPrimeiroArquivo.replace(
                extensao,
                `${extensaoAlvo}`
              ),
              resultado,
